...
local filter = adi:RegisterFilter('BetterCategories', 90, 'ABEvent-1.0')
...
function filter:GetClassIds(itemId)
    local entry = data.overrides_wrath[itemId]

    if entry == nil then
        entry = data.overrides_tbc[itemId]
    end

    if entry == nil then
        entry = data.overrides_vanilla[itemId]
    end
